...
var SchemaFromExport = /** @class */ (function () {
    ...
    SchemaFromExport.prototype.handle = function (file, cliOptions) {
        var _this = this;
        graphql_codegen_core_1.logger.info("Loading GraphQL schema object, text, ast, or introspection json from JavaScript ES6 export: " + file + "...");
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        if (!fs_1.existsSync(fullPath)) return [3 /*break*/, 14];
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 12, , 13]);
                        exports_1 = require(fullPath);
                        if (!exports_1) return [3 /*break*/, 10];
                        rawExport = exports_1.default || exports_1.schema || exports_1;
                        if (!rawExport) return [3 /*break*/, 8];
                        schema = void 0;
                        if (!this.isPromise(rawExport)) return [3 /*break*/, 3];
                    ...
                    case 6:
                        e_1 = _a.sent();
                        graphql_codegen_core_1.debugLog('Unexpected exceptiom while trying to figure out the schema: ', e_1);
    ...
    SchemaFromExport.prototype.resolveSchema = function (schema) {
        ...
        return new Promise(function (resolve, reject) {
            if (_this.isSchemaObject(schema)) {
                resolve(schema);
            }
            else if (_this.isSchemaAst(schema)) {
                resolve(graphql_1.buildASTSchema(schema));
            }
            else if (_this.isSchemaText(schema)) {
                var ast = graphql_1.parse(schema);
                resolve(graphql_1.buildASTSchema(ast));
            }
            else if (_this.isSchemaJson(schema)) {
